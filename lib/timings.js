...
const currentMonarchCode = "T",
    currentMonarchHebrewYearDiff = 5774;
const roshHashanahMonthNo = 7;
(indexOfNisan = 8), (noOfHebrewMonths = 13);
...
const daysInAWeek = 7,
    ...
    monthsInAYear = 12,
    centuryInterval = 100;
const allSaintsEve = [31, 9],
    allSaints = [1, 10],
    allSouls = [2, 10],
    earliestAdventSunday = [27, 10],
    christmas = [25, 11];
const daysInLent = 46,
    easterToAscension = 39,
    easterToPentecost = 49,
    daysOfChristmas = 12,
    daysInFirstWeekOfLent = 4;
...
class Timings {
    ...
    getCyprian() {
        ...
        var result = this.packCyprianDate(
            cyprianDate.dayNum,
...
class CyprianDate {
    ...
    getCyprianYear() {
        ...
        if (this.monthNum >= indexOfNisan) result = result + 1;
...
class SacredDate {
    ...
    buildAdventSunday() {
        var christmasWeekday = sakamotoGetWeekday(
            this.gregorian.year,
            christmas[1] + 1,
            christmas[0]
        );
        ...
        result = SacredDate.addDays(
            earliestAdventSunday,
            n,
            this.gregorian.year
        );
    ...
    buildSeason() {
        ...
        if (this.seasonal === "Advent") {
            result = new Advent(this.gregorian, this.adventSunday);
        } else if (this.seasonal === "Christmas") {
            result = new Christmas(this.gregorian, this.epiphany);
        } else if (this.seasonal === "After Epiphany") {
            result = new AfterEpiphany(this.gregorian, this.epiphany);
        } else if (this.seasonal === "Lent") {
            result = new Lent(this.gregorian, this.ashWednesday, this.easter);
        } else if (this.seasonal === "Easter") {
            result = new Easter(this.gregorian, this.easter, this.pentecost);
        } else result = new AfterPentecost(this.gregorian, this.pentecost);
...
class Advent {
    ...
    buildWeek() {
        var today = [this.gregorian.day, this.gregorian.month];
        var daysAfterAdventSunday = SacredDate.daysAfter(
            this.adventSunday,
            today,
            this.gregorian.year
        );
        var weekNo =
            daysAfterAdventSunday - (daysAfterAdventSunday % daysInAWeek);
...
class Christmas {
    ...
    buildDayOfChristmas() {
        var today = [this.gregorian.day, this.gregorian.month];
        var daysAfterChristmas = SacredDate.daysAfter(
            christmas,
            today,
            this.gregorian.year
        );
    ...
    buildDateString() {
        var today = [this.gregorian.day, this.gregorian.month];
        var result =
            SacredDate.nToNth(this.dayOfChristmas) + " day of Christmas";
...
class AfterEpiphany {
    ...
    buildWeek() {
        // This assumes that we're using the definition of Epiphany wherein
        // it always falls on a Sunday.
        var today = [this.gregorian.day, this.gregorian.month];
        var result = SacredDate.daysAfter(
            this.epiphany,
            today,
            this.gregorian.year
        );
...
class Lent {
    ...
    buildWeek() {
        var today = [this.gregorian.day, this.gregorian.month];
        var daysAfterAshWednesday = SacredDate.daysAfter(
            this.ashWednesday,
            today,
            this.gregorian.year
        );
    ...
    buildColour() {
        ...
        if (
            SacredDate.fallsOn(this.palmSunday, today) ||
...
class Easter {
    ...
    buildWeek() {
        var today = [this.gregorian.day, this.gregorian.month];
        var daysAfterPentecost = SacredDate.daysAfter(
            this.pentecost,
            today,
            this.gregorian.year
        );
    ...
    buildDateString() {
        ...
        if (SacredDate.fallsOn(this.easter, today, this.gregorian.year)) {
            ...
            SacredDate.fallsOn(this.ascension, today, this.gregorian.year)
        ) {
            result = "The Feast of the Ascension";
        } else if (
            SacredDate.fallsOn(this.pentecost, today, this.gregorian.year)
        ) {
