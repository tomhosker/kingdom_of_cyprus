...
class ChivalricMaker
{
  ...
  getMaster()
  {
    var result = utils.makePersonLink(this.data.Chivalric.masterShortTitle,
  ...
  getMembers()
  {
    ...
    {
      holder = utils.makePersonLink(raw[i].shortTitle, raw[i].person,
                                    raw[i].style, raw[i].rankTier);
...
function holdersDeepCopy(item)
{
  ...
  for(var i = 0; i < item.holders.length; i++)
  {
    ...
    if(i !== item.holders.length-1) newHolders = newHolders+", ";
  }
