...
class AllScraper
{
  ...
  doAllPerson(req, res)
  {
    var key = "code", descriptor = "shortishTitle", tableName = "Person";
    var preKey = "/people/";
...
class CountyScraper
{
  ...
  fetchCounty(req, res, code)
  {
    var query = "SELECT County.*, "+
                  ...
                  "LordLieutenant.shortTitle AS lordLieutenantShortTitle, "+
                  "LordLieutenant.style AS lordLieutenantStyle, "+
                  "LordLieutenant.rankTier AS lordLieutenantRankTier "+
                "FROM County "+
...
class BaronyScraper
{
  ...
  fetchBarony(req, res, code)
  {
    var query = "SELECT Barony.*, "+
                  ...
                  "KnightLieutenant.rankTier AS knightLieutenantRankTier "+
                "FROM Barony "+
                "JOIN Person Baron ON Baron.code = Barony.baron "+
                "JOIN Person KnightLieutenant ON "+
                  "KnightLieutenant.code = Barony.knightLieutenant "+
                "WHERE Barony.code = ?;";
    var db = new sqlite3.Database(pathToDB);
    var data = {};
    var that = this;
...
class ChivalricScraper
{
  ...
  fetchHolds(req, res, code, data)
  {
    var query = "SELECT Holds.person, Holds.accolade, Person.code, "+
                  "Person.shortTitle, Person.style, Person.rankTier, "+
                  "Accolade.name "+
                "FROM Holds "+
...
class GovScraper
{
  ...
  fetchGovernment(req, res)
  {
    var query = "SELECT Government.*, "+
                  ...
                  "Remembrancer.shortTitle AS remembrancerShortTitle, "+
                    "Remembrancer.style AS remembrancerStyle, "+
                    "Remembrancer.rankTier AS remembrancerRankTier, "+
                  ...
                  "MasterOfOffices.shortTitle AS "+
                    ...
                    "MasterOfOffices.style AS masterOfOfficesStyle, "+
                    "MasterOfOffices.rankTier AS masterOfOfficesRankTier, "+
                  "MasterOfHorse.shortTitle AS "+
                      "masterOfHorseShortTitle, "+
                    "MasterOfHorse.style AS masterOfHorseStyle, "+
                    "MasterOfHorse.rankTier AS masterOfHorseRankTier, "+
                  "MasterOfTheRolls.shortTitle AS "+
                      "masterOfTheRollsShortTitle, "+
                    "MasterOfTheRolls.style AS masterOfTheRollsStyle, "+
                  ...
                  "MasterOfAlms.shortTitle AS masterOfAlmsShortTitle, "+
                    "MasterOfAlms.style AS masterOfAlmsStyle, "+
                    "MasterOfAlms.rankTier AS masterOfAlmsRankTier, "+
                ...
                "LEFT JOIN Person MasterOfOffices "+
                  "ON MasterOfOffices.code = Government.masterOfOffices "+
...
class AcademyScraper
{
  ...
  fetchLeadership(req, res, data)
  {
    var query = "SELECT AcademyProperty.*, Person.code AS personCode, "+
                  "Person.style AS personStyle, Person.shortTitle AS "+
                  "personShortTitle, Person.rankTier AS personRankTier "+
...
class FacultyScraper
{
  ...
  fetchFaculty(req, res)
  {
    var query = "SELECT Faculty.*, Person.code AS personCode, "+
                  "Person.shortTitle AS personShortTitle, Person.style AS "+
                  "personStyle, Person.rankTier AS personRankTier "+
                "FROM Faculty "+
...
class FacultyDepartmentScraper
{
  ...
  {
    var query = "SELECT FacultyDepartment.*, Faculty.code AS facultyCode, "+
                  "Faculty.name AS facultyName, Person.shortTitle AS "+
                  "personShortTitle, Person.code AS personCode, "+
                  "Person.style AS personStyle, Person.rankTier AS "+
                  "personRankTier "+
...
class HCinemaScraper
{
  ...
  fetchBookI(req, res)
  {
    ...
    var query = "SELECT * FROM Film "+
                "WHERE ((genre = 'comedy') AND (rank <= "+maxRank+") AND "+
  ...
  fetchBookII(req, res, data)
  {
    ...
    var query = "SELECT * FROM Film "+
                "WHERE ((genre = 'tragedy') AND (rank <= "+maxRank+") AND "+
                       "(rank > "+hCinemaBookVPartLength+")) "+
                "ORDER BY rank DESC;";
    var db = new sqlite3.Database(pathToCanons);
    var that = this;
  ...
  fetchBookIII(req, res, data)
  {
    ...
    var query = "SELECT * FROM Film "+
                "WHERE ((genre = 'satire') AND (rank <= "+maxRank+") AND "+
                       "(rank > "+hCinemaBookVPartLength+")) "+
                "ORDER BY rank DESC;";
    var db = new sqlite3.Database(pathToCanons);
    var that = this;
  ...
  fetchBookIV(req, res, data)
  {
    ...
    var query = "SELECT * FROM Film "+
                "WHERE ((genre = 'other') AND (rank <= "+maxRank+") AND "+
                       "(rank > "+hCinemaBookVPartLength+")) "+
                "ORDER BY rank DESC;";
    var db = new sqlite3.Database(pathToCanons);
    var that = this;
  ...
  fetchBookVPart2(req, res, data)
  {
    var maxRank = hCinemaBookVPartLength;
    var query = "SELECT * FROM Film "+
                "WHERE ((genre = 'tragedy') AND (rank <= "+maxRank+")) "+
                "ORDER BY rank DESC;";
    var db = new sqlite3.Database(pathToCanons);
    var that = this;
  ...
  fetchBookVPart3(req, res, data)
  {
    var maxRank = hCinemaBookVPartLength;
    var query = "SELECT * FROM Film "+
                "WHERE ((genre = 'satire') AND (rank <= "+maxRank+")) "+
                "ORDER BY rank DESC;";
    var db = new sqlite3.Database(pathToCanons);
    var that = this;
